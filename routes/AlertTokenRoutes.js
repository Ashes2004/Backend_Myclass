...
router.post('/send-notification/:id', async (req, res) => {
  ...
  try {
    ...
    // Find the token to exclude
    const selfTokenDoc = await AlertToken.findById(req.params.id);
    if (!selfTokenDoc) {
      return res.status(400).json({ error: 'No registered AlertToken with the given ID' });
    }
    
    const selfToken = selfTokenDoc.token;
    const updatedAlertTokenArray = AlertTokenArray.filter(token => token !== selfToken);

    if (updatedAlertTokenArray.length === 0) {
      return res.status(400).json({ error: 'No tokens left to send notifications to' });
    }

    // Function to send notifications
    async function sendNotifications() {
      for (const token of updatedAlertTokenArray) {
        const message = {
          notification: {
            title: title || 'Test Notification',
            body: body || 'This is a test message',
    ...
    console.error('Error in /send-notification/:id:', error);
